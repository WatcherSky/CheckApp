...
extension CountriesListViewController: UICollectionViewDataSource {
    func collectionView(
        ...
        let country = viewModel.sortedCountries[indexPath.section][indexPath.item]
        cell.configure(with: country)
        cell.learnMoreHandler = { [weak viewModel] in
            viewModel?.selectCountry(cca2Code: country.cca2Code)
        }
        return cell
    }
